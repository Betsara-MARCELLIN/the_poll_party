  ...
  void initState() {
    ...
    accel = userAccelerometerEvents.listen((UserAccelerometerEvent event) {
      if (event.x > 2 || event.y > 2 || event.z > 2) {
        setState(() {
          if(cursorValue<widget.maxCursorValue)
          cursorValue++;
        });
      }
    });
    ...
    return Column(
      children: [
        ...
        Padding(
          ...
          child: Text('Question: ${widget.socketProvider.getCurrentQuestion().question}',
        ...
        Text("$cursorValue",style: TextStyle(fontSize: 30, fontWeight: FontWeight.bold, color: Theme.of(context).primaryColor)),
        ...
        Padding(
          ...
          child: Text("Secouez votre smartphone pour augmenter la valeur", style: TextStyle(fontStyle: FontStyle.italic, color: lightBlack), textAlign: TextAlign.center,),
        ),
      ],
    );
  }
